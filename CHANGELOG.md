...
- Agents/xAI web_search tool collision: drop OpenClaw's local `web_search` function tool when the active model provider is xAI/Grok (direct or via OpenRouter `x-ai/*`), since those providers reserve a native `web_search` tool and reject requests that also define a duplicate tool name; `web_search` stays registered for all non-xAI providers.
